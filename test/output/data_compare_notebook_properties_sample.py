...
#the sample size (can be integer or float <= 1.0)
sample_size =
...
if sample_size <= 1.0:
    ...
    if sample_size > np.min([table1.shape[0], table2.shape[0]]):
        raise ValueError("sample_size: should be smaller or equal to len(table1) and len(table2)")
...
df1 = table1[[col]].copy().sample(sample_size).reset_index(drop=True)
df2 = table2[[col]].copy().sample(sample_size).reset_index(drop=True)
